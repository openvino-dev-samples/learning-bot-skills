...
| PR5 | "用 GPU 识别这张图里的文字。" | PRESET / `ocr-npu` | 清单里只有 NPU 版 OCR；点名 GPU 也只能给 `ocr-npu`，不要造一个不存在的 skill。 |
...
|---|---|---|---|
| ST1 | "启动 learning bot。" | 输出预设问题清单（`-Menu`） | 推荐 15 条预设问题（附示例话术）；说明超范围时会转开发类 skill；不臆测具体任务。 |
| ST2 | "你能做什么 / 有哪些功能？" | 输出预设问题清单 | 同 ST1；列出 15 个本地能力 + 3 个开发类 skill 的适用场景。 |
...
| NG4 | "把 15 个本地 skill 一次性全装到我电脑上。" | 先确认收窄 | 这些 skill 已上架、按名字调用即可；不要盲目全量拉起，先请求确认或收窄范围。 |
...
- **预设本地能力：** PR1–PR17 —— 15 个已上架 aipc-skill 全覆盖
