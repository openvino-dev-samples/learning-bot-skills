...
在进入构建/优化主流程前，本技能可以先给用户**一组准备好的问题**，让用户确认前置条件或收敛意图
（截图里的「§2 前置条件 多选」就是 `preflight`）。三类问题都以统一的 `[SKILL_QUESTIONS]` 契约输出
（离线、无需 venv/克隆）：

```powershell
run.ps1 --questions preset      # 推荐问题："你可以让我做这些"
run.ps1 --questions preflight   # 前置条件多选（硬件/工具链/模型/notebook 是否就绪）
run.ps1 --questions clarify     # 澄清追问（slug vs goal、设备/精度、是否 --serve、--china）
...
**agent 约定：** 进入主流程前先走 `--questions preflight`，把它渲染成多选清单让用户勾选；对**没勾**的项按
`on_missing` 先补齐（`openvino-environment-management` 配环境、`openvino-content-fetch` 下模型，
`self:--dry-run` 表示本技能自身的动作），勾了 `exclusive` 的「以上均完成，无需引导我」则跳过引导直接进入
主流程。问题清单定义在 [scripts/questions.json](scripts/questions.json)，由共享的
